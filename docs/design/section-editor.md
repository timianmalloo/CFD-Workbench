...
**B6 deferred past M1.1 (2026-09-25, repair cap fired: 2 of 2 cycles).** Neither cycle committed code; both measured the obstacle. Per-query overlay subdivision closes max_x T0 only to about 1e-7 within 256 nodes and 1 s, because rational bit size grows with depth. A uniform 1e-12 grid needs about 10⁶ leaves per profile; best-first refinement reached 3.3e-10 after 2,048 splits for a +0.02 CV x move and 2.8e-6 at 256 nodes for +0.20. Effect: an x edit on an independent profile whose neighbour has a different abscissa basis is refused with `DSL-GEOMETRY` naming the neighbour; all other edits and constructions certify. Recommended restart point: adaptive refinement once at admission (dyadic de Casteljau cuts at span-endpoint abscissae and overlap midpoints of the atom owning the blend upper bound, covered slivers, frozen atoms stored on the certificate), refusing only pairs that exhaust the admission budget or the 32,768-bit rational ceiling.
