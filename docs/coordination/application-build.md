...
**Later gate, 2026-09-23:** Rulings 17–19 choose evaluator `/2` degree-CV identity, whole-query deterministic feasibility, and a fixed directory-relative cooperative overwrite claim with measured native I/O. They authorize repair, not B acceptance. Root has the [29-path exact companion assignment](contract-r17-companions.md) in a disjoint tree while the serial core author finishes its `/1` store/Ruling 16 checkpoint. The reviewed companion commit must precede a dedicated `/2` core continuation; C remains held.
