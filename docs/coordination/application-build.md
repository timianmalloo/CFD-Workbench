...
| C | 90 calls / native UI machine time and disjoint files after accepted B | Clean requested-Sol/effective-Not-recorded candidate `6f168c44` after two bounded interaction repairs; aggregate calls/tokens Not recorded | The initial `de105f0` package passed 11 steps. Root's actual n1/fixed-copy CUA review found and retested keyboard/focus defects, then identified tab-region, selected-CV activation, numeric binding and recovery gaps. Targeted repair tests passed at `6f168c44`; full packaging was reserved for the Ruling 27 combined review. C production remains unjoined. |
| D | deterministic join / independent integrated proof | B product/review integrated at `18278c4`; B permission `ee6d73ad` and C `6f168c44` repairs remain isolated; independent review joined docs-only at `77cb6f6`; exact aggregate calls/tokens Not recorded | Ruling 27 allowed one disposable B+C review package. Its 11-step gate and source/package/launch identity checks passed. The first supported CUA bind to that package returned `cgWindowNotFound`, so actual packaged native Save and repaired interactions remain unassessed; no B/C product join or full M1 claim. Windows runtime and distribution trust remain separate. |

| status | item |
|---|---|
| Completed | Pack/repo graph, bounded architecture/B0 and independently accepted original B; B joined at `18278c4`. Clean B permission repair `ee6d73ad` passed changed-source and independent public-store mode checks. Clean C repair `6f168c44` passed targeted tests. Ruling 27 review-only composition `e6e5628` passed one 11-step combined gate; root verified its source/helper/launch binding and the independent review joined as documents at `77cb6f6`. |
| Remaining | The combined app PID 71600/start `Wed Sep 23 15:00:25 2026` is live, but root's first supported exact-path CUA bind returned `cgWindowNotFound`. Root has asked the user to bring this newest review window forward; the answer is pending. Actual packaged native Save/Reopen/`0600`, the repaired keyboard/recovery interactions, B permission Owner gate, both production joins, Windows runtime and release trust remain open. |
| Best next action | On confirmed visibility of the exact combined review window, root alone completes the pending CUA checklist and native Save/Reopen/mode readback. If CUA remains unavailable, retain the isolated app/receipts and blocked disposition without another launch variant. A root independent verdict precedes fresh Owner B/C gate requests and any supported conductor product join. |

### Native visibility continuation · one optimized graph

The user replied, verbatim, “Yes, I see the workbench” and “The new CFD Workbench review window”. Root's supported CUA attachment then returned a rendered window and AX tree bound to PID 48841, start `Wed Sep 23 13:24:28 2026`, Desktop DLL `6aaff522eec3da85898685992fd3f2e3325035e2c482b9edda76b6dc42f54aa9`. This is the earlier n1 review copy; it is not a bitwise claim for the final e7 development package. Root's [native review](../reviews/ui-application-native.md) records the actual action/result rows. The CUA surface exposed a screenshot and AX return to the reviewer, without a documented file export; no PNG or AX file is claimed.
...
The [review-copy manifest](/tmp/cfd-combined-review-20260923.muHX8h/review-copy-manifest.json), SHA-256 `4bd4cea5765d4e79d7cbe0aea1a09cbb1463a3f0c0c4627366434728b631840b`, compares 231 files and proves only `Info.plist` `CFBundleIdentifier` changed to `com.cfdworkbench.desktop.reviewcombined27`. The [launch receipt](/private/tmp/cfd-combined-review-20260923.muHX8h/launch-receipt.json), SHA-256 `b3679315d3116bd1423b6899dddd5748e719dea09573ee81b4f4f146e64bd3c8`, binds live PID 71600/start `Wed Sep 23 15:00:25 2026`, exact executable and `Window.Opened` marker to that copy. Root independently checked source/helper/PID facts but CUA returned `cgWindowNotFound` on its first exact-path bind. The previously observed native keyboard/recovery defects and the old `0454` Save are not cleared by these build or startup facts. Root's [independent review](../reviews/ui-application-native.md) retains the native veto until actual behavior is observed. The review tree/package/file receipts stay available; canonical B/C joins remain held.
