...
| C · native adapters, `cfd-adapters-20260923` | [Exact 24-path contract](contract-c-native.md), no core implementation files; Ruling 29 narrows the current continuation to nine existing paths | Joined B `a550466`, independently reviewed [compiled API freeze](contract-c-api-freeze.md), Owner Rulings 21 and 29 | T2 | 0 | Initial 90 calls / 55 minutes; Ruling 29 first checkpoint ≤60 calls / 35 minutes, aggregate calls/tokens Not recorded | Candidate `6f168c44` in review-only B+C `e6e5628` passed 11 adapter steps; root verified named native keyboard, edit, recovery, import and Save/Reopen flows. Isolated C handoff `13c883a` exposed two portable-tooling gate failures; targeted repair is in flight | Requested Sol, effective model Not recorded. Root UX/performance veto and C/M1 product join remain open; no Windows runtime claim. |
| D · integration and independent proof | Coordinator owns merge and plan/status ledger; root/Owner own independent review/rulings, no production co-authoring | B accepted; C source-bound native evidence exists but timing/state coverage remains open | T2 | 0 | One affected recount per eligible product join; docs-only joins do not clear product gates | B permission repair and independent review joined at `a550466`; final root native review joined docs-only at `b3e48aa`. Downstream C handoff is isolated and held at a portable-tooling RED | Deterministic scripts, root independent veto and Owner Rulings 28–29. |
...
| C | Native UI machine time and disjoint files after accepted B; Ruling 29 ≤60 calls / 35 minutes first continuation checkpoint | Clean requested-Sol/effective-Not-recorded `6f168c44`; review-only combined `e6e5628`; downstream handoff `13c883a`; aggregate calls/tokens Not recorded | Root CUA verified named keyboard/edit/recovery/import and native Save/Reopen rows after user brought the exact combined window forward. `13c883a` has identical `src/tests/tools` bytes to reviewed combined source but conductor step 8 found three portable-text and two subprocess-encoding findings. The author reproduced 3+2 RED and targeted GREEN in two C tools; render-endpoint spike and measured native state/timing proof remain. No C product join. |
| D | deterministic join / independent integrated proof | B product/review integrated at `a550466`; root final native review joined docs-only at `b3e48aa`; exact aggregate calls/tokens Not recorded | Rulings 28–29 separate completed B permission scope from open C UX/performance. A new isolated C continuation starts from the failed handoff; first two portability controls are repaired locally, with endpoint spike review before timed trials. Windows runtime and distribution trust remain separate. |
...
| Remaining | C remains isolated. Its clean downstream handoff `13c883a` failed two integrated portability gates on five C-tooling findings; Ruling 29's targeted repair is in progress. The actual app lacks end-to-visible 5-second cold, 100 ms edit, and 250 ms preview/cancel measurements and the full declared native state/theme/dense review. Windows runtime/store and release trust are unassessed. |
| Best next action | The C author finishes targeted portable-tooling controls, then sends an installed-Avalonia render-endpoint spike for root review before any timed trial collection. Run bounded normal-path timing and source-bound state matrix only after that endpoint is judged honestly; keep C product join and M1 acceptance held for independent root/Owner disposition. |
