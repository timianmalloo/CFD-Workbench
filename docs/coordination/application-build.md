...
| B · accepted-source core | [Provisional exact 18-path packet](contract-b-core.md), including owned proof/gate; no UI files | B0 independent PASS and Owner G3 freeze | T2 | 0 | 90 calls, ≤100k context, ≤55 min first run | Red→green full-language recognition, accepted certified subset, immutable revisions, semantic/source identity, persistence fault/reopen proof, CLI-consumable API, emitted normal-path events, descendant commit | Ruling 11 conditionally selects one serial built-in Codex core author with requested Astra/effective Not recorded and observed-only controls. An actual harmless same-harness cancellation/quiescence drill, exact cwd/HEAD/paths/caches and B0/G3 gates precede dispatch. Current Grok ACP remains unqualified; no new probe. |
