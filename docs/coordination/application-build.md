...
| status | item |
|---|---|
| Completed | B original core and macOS permission repair are joined at `a550466`. Isolated C R39 `0d4a590` passed 396 applied-control rows, 21 negatives and a source-bound 12-step gate; root independently passed the exact High Contrast focused-field and tab-hover native regressions. The two-fresh-build CUA readiness control and its bounded review joined. Windows W0 and the first actual Windows x64 hosted run have retained receipts; the latter failed qualification. R47 source-only timing preparation and independent preparation review are joined. R50's independently accepted owned-conductor wrapper is joined and exercised on the R50, wrapper and R47 joins with live epoch/readback. The user placed Windows and actual visible timing in M1 and deferred full section authoring to a new session. |
| Remaining | C product remains isolated pending a technical integration disposition; its native contrast pass does not clear displayed timing, Windows runtime, broad assistive technology, signing or release trust. The Windows run failed four of 26 native cases, left directory durability Not assessed, and stopped on a DACL receipt-oracle mismatch before UIA. The frozen R48 local correction remains partial because final-arm identity failure is not fail-closed; no new remote run is eligible. W2 product runtime and W3 independent desktop proof remain open. R47 qualifies only noncapture preparation: native target identity, geometry, outer timeout, capture authorization and visible budget trials remain open. The CUA internal failure cause remains inferred. |
| Best next action | Obtain a narrow Owner ruling for R48 final-arm containment and cleanup proof, then independent review before a new Windows dispatch. In parallel, qualify R47's noncapture native identity/geometry and outer timeout contract to make one exact target-only capture request reviewable. Keep the C technical join and all unmeasured gates explicit. Do not start the full section editor in this session. |
...
### Rulings 30–37 · repaired C checkpoint and native wait

| Work node | Planned exit | Observed actual |
|---|---|---|
| Applied theme repair | Loaded XAML and actual painted control pairs for Light, Dark, High Contrast and Default, including focus and modal; 72 required rows, text ≥4.5:1 and focus ≥3:1 | Frozen Dark and High Contrast windows exposed root resource shadowing. The repaired isolated C commit `5bdf0e7` passed all 72 actual-control rows; independent root minimums were 6.28577:1 text and 4.01698:1 focus. Root's [review](../reviews/ui-application-native.md) records the R30–37 failures, fixes and limits. |
| Same-process native lifetime | All four windows complete without a disposed-session read; unsaved-close Cancel stays live | Actual two-window stack reached queued `Refresh` after `Closed` and threw `DOC-CLOSED`. The R37 fix guards already-queued callbacks while retaining the core refusal. The targeted regression and same-process 72-row matrix passed. |
...
### R38 contrast repair, attachment readiness and approved M1 placement

The R37 High Contrast copy revealed a repeatable selected FoilDSL tab label
failure under pointer hover. Ruling 38 authorized only four C author paths.
The isolated author committed `d774e1e` after an actual loaded-control RED,
an expanded 312-row Light/Dark/HighContrast/Default styled-state matrix and
one [changed-source 12-step gate](/private/var/folders/8b/b13cycfj2psdxdnk19xw8jch0000gn/T/cfd-adapters-verify-jit8fbuw/receipts/verification.json).
Coordinator independently recomputed 78 unique rows per theme, 240 state
facts, minimum text contrast 6.28577:1 and focus contrast 4.01698:1, then
checked all 53 stable source inputs, 16 binary hashes, logs and owned-process
quiescence. These are loaded-control and package results; native pointer and
screen-presentation behavior remain separate.

The optimized continuation has two independent evidence branches and three
new proof tracks following the resolved human-decision edge:

| Node | Dependency | Exit and stop condition |
|---|---|---|
| Part 1 · review-window attachment readiness | Root owns a separate isolated investigation/control, with Coordinator independent SRE/Test review. The historical `cgWindowNotFound` platform cause has not been reproduced. | An exact launch/AX/nonempty-screenshot receipt and two genuinely fresh source-bound build trials may make native review repeatable. A bounded timeout or unknown process blocks only that native node; it is not a platform-root-cause fix or a product acceptance claim. No user foreground request substitutes for a missing receipt. |
| R38/R39 native contrast | Frozen C `d774e1e` and a reviewed attached app; one task-owned window at a time after the launch guard. | Native High Contrast selected-tab hover passed twice, but focused numeric text was unreadable. [Owner Ruling 39](../notes/rulings.md) authorizes a bounded measurement-first correction of the actual TextBox sibling backdrop and its oracle. Root retains the UI veto; a passing styled-state matrix cannot clear it. |
| Windows x64 · M1 | [User-approved placement](../notes/m1-scope-decision.md); existing macOS B and C candidate contracts are the starting point, not Windows runtime proof. | Qualify a real Windows x64 execution route, then design/author the persistence/process seams and run native UIA/Narrator, file and numerical parity checks. Hosted CI can cover unattended executable tests; a separately qualified interactive route is still needed. A Windows build alone does not pass. |
| Visible timing · M1 | User-approved unchanged cold/edit/Preview/Cancel budgets and a correct final visible endpoint. | Establish a source-bound on-screen measurement and reference fixture/host before interpreting p95 or optimizing. The existing 93 batch-cycle diagnostics remain distinct. Unknown endpoint is Not assessed, not a budget pass. |
| Section authoring · M1.1 | User-approved named next increment, independent of M1 acceptance. | Write a detailed multi-profile/data/scientific/UX design and obtain independent Data, scientific, UX and Test review before assigning implementation. The narrow M1 rail edit remains narrow. |

The placement decision creates planning authority, not overlapping file leases.
The Coordinator owns this ledger and dispatch order; Owner freezes typed seams,
exact paths and budgets before each new author. Root independently reviews each
platform, timing or section claim and retains the native veto. The next seats are:

| Track and owner | Required predecessor | First dispatchable result | Later acceptance edge |
|---|---|---|---|
...
production paths. Fresh-build trial 1 used the R38 full-gate package; a second
isolated macOS publish/package used the same 53 source bytes and distinct
scratch outputs. Its raw runner initially marked FAIL solely because it
required bitwise equality to trial 1's precommit DLLs, an extra condition not
in the two-build request. The retained raw receipt shows both build/package
steps exit zero and quiescent; a separate
[assessment](/private/var/folders/8b/b13cycfj2psdxdnk19xw8jch0000gn/T/cfd-r38-attach-build2-lr233fn2/receipts/fresh-build2-assessment.json)
records the same 53 source hashes and exact binary differences. The four DLLs
and four PDBs differ between precommit and committed builds; no bitwise
equivalence or sole causal explanation is claimed. Root's first build-2 CUA
...
### R50 adoption and R47/R48 checkpoint

Owner [Ruling 50](../notes/rulings.md#ruling-50--accept-corrected-macos-live-leader-preflight-for-bounded-adoption) accepted the exact R49 wrapper for bounded macOS use after independent Coordinator and Owner checks. The Coordinator pinned fresh live leader epoch 21 and used the hash-verified absolute wrapper for the R50 bootstrap join, then the joined `tools/run-owned-conductor.py` for subsequent joins. The R50 join receipt `/tmp/cfd-r50-bootstrap-owner-20260925-epoch21.jsonl`, R49 source continuation `/tmp/cfd-r49-source-continue-20260925-epoch21.jsonl`, R47 source join `/tmp/cfd-r47-source-join-20260925-epoch21.jsonl` and R47 independent-review continuation `/tmp/cfd-r47-review-continue-20260925-epoch21.jsonl` each retained two strict JSON events, matching live epoch/context, zero refusal, conductor exit zero and 11 passing verify gates. The two generated-index conflicts were resolved by `docs-graph.py derive`, never by choosing a branch's generated hash. This is observed project preflight use; it does not make leadership atomic or qualify the wrapper on Windows.

R47's five-path noncapture identity preparation `657cd8c` and root's independent review `974a39d` are integrated. The author run measured 867 seconds, audit `al-01M3BM89P00Y5AS3NPQKWP7DWA`; root's separate review measured 437 seconds in its own audit. The 78 pure controls, four mutants, private 31-file/27-dependency package closure and two durable proof capsules support preparation only. Native Swift target lookup, actual geometry and file guards, an outer deadline/cleanup, on-screen visibility and budget trials remain unassessed. A later exact target-only capture request still requires specific user authorization after the route is concrete.

R48's frozen four-path local checkpoint `ab28bdf` measured 705 seconds and 22 author calls, audit `al-01M3BM8GSQMXRPR0CS7880K0SC`. Its 102 consumer controls, six apphost injections, five mutations and local compile/portability/docs checks pass. The final diagnostic arm can still reach a general failure after an unsupported pin/identity observation; its original arm might then continue without proven final-arm containment. The author stopped at its correction cap and recorded this as a prepush blocker. No R48 source join, Windows rerun or product acceptance follows. The next Owner decision must require a fail-closed arm-safety classification, actual fault control, and constructor-on-throw owned-handle cleanup evidence before independent prepush review.
