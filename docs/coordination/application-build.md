...
| Remaining | The user confirmed the exact new review window visible, and root attached through supported CUA to the source-bound n1 review copy. Native inspection found F6 skipping Navigator, focus loss on list selection, and stale numeric/re-edit state after Cancel. These are observed native vetoes. Root's remaining interaction checklist, a fixed-package native recheck, Owner gate and any C product join remain open. Windows runtime and release trust remain separate. |
| Best next action | The same isolated C author repairs the three measured interaction failures in the exact MainWindow/test paths. Root retains the frozen instance as independent evidence, then tests a new source-bound package through CUA. Request Owner disposition only after that review; C stays unjoined meanwhile. |
