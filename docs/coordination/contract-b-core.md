...
| Initial budget and checkpoints | 90 tool calls, ≤55 minutes, ≤100k context for first run. Checkpoints after red boundary fixtures, compiling core API, and store fault tests. At cap, report/replan without claiming completion. No author subagents. |
...
names against the final B0 public namespace and fixture. The worker may not
...
than claiming atomicity. The [architecture's normal-path event contract](../architecture/application.md)
...
Avalonia/UI and network dependencies.
