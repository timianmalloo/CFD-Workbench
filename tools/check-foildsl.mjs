...
try{
  ...
  const properties=await page.evaluate(()=>{let tested=0;for(let i=1;i<=64;i++){const r=drecord();r.half=.125+i/128;r.cv.chord[3][1]=.04+i/1024;const out=demit(r);if(dmeaning(dparse(out))!==dmeaning(r)||demit(dparse(out))!==out)throw Error('round-trip failure '+i);tested++;}return tested;});
  ...
  await page.locator('#dsl-source').fill(comment.source+'\n# pending');await page.locator('#dsl-geometry').click();const guard=await page.evaluate(()=>{const before=JSON.stringify(M.cv);cvMove('chord',3,M.cv.chord[3][0],.5);addStation(.42);return {same:before===JSON.stringify(M.cv),noStation:!M.stations.includes(.42),preview:M.preview};});assert(guard.same&&guard.noStation&&!guard.preview);
