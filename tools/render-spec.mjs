...
const isLanguage = specName === 'foildsl';
const revision = isLanguage ? body.match(/^# FoilDSL ([0-9.]+)\s*$/m)?.[1] : body.match(/^Product specification · revision ([0-9.]+) ·/m)?.[1];
...
content=content.replaceAll('href="foildsl.md"','href="foildsl.html"');
const ids=[...new Set([...body.matchAll(/\b(?:DOC|GOAL|GEO|CAT|ANA|DRC|LAB|CFD|VIZ|EXP|AI|CLI|SET|CAD|XS|RUN|RES|CAND|UX|UI|SRC|DSL)-\d{2}\b/g)].map(x=>x[0]))];
for(const id of ids) if(!content.includes(id)) throw new Error(`Requirement omitted: ${id}`);
const html=`<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="source-sha256" content="${sha}"><title>${escape(documentTitle)}</title>
<style>
