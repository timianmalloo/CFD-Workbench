...
public sealed class SessionAssessment
{
    internal SessionAssessment(Guid owner, GeometryStatus status, string code, SessionBinding? key, GeometryCertificate? certificate,
        AuthoredBinding? sourceBinding = null, IEnumerable<Diagnostic>? diagnostics = null, ThicknessProposal? thickness = null)
    {
        Owner = owner; Status = status; Code = code; Key = key; Certificate = certificate; SourceBinding = sourceBinding;
        Diagnostics = Array.AsReadOnly((diagnostics ?? []).ToArray()); Thickness = thickness;
    }
        ...
        try
        {
            ...
            if (thickness?.Fault is string fault)
                return new(authorityId, fault == "DSL-LOCK" ? GeometryStatus.Invalid : GeometryStatus.NotAssessed, fault, key, null,
                    DraftBinding(capture, parsed), [ThicknessDiagnostic(capture, fault)], thickness.Value.Proposal);
            ...
            return new(authorityId, result.Status, result.Code, key, result.Certificate, DraftBinding(capture, parsed), diagnostics, thickness?.Proposal);
        }
